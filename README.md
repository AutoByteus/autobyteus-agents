...
The product iteration team is the PM-first entrypoint for continuous product improvement. It starts with `product_manager`, turns a product goal into a Product Iteration Plan with an ordered backlog/cursor, routes exactly one Product Feature Brief at a time to `solution_designer` for normal Engineering Intake / Stage 0, and expects `delivery_engineer` to send an acceptance packet to `product_manager` after truthful delivery evidence. Product Manager can return `Accepted` / `Needs Rework` / `Blocked`; only Product Manager `Accepted` is the product-loop verification signal for product-iteration archival/finalization, while Acceptance Callback Status records packet delivery only. After acceptance, Product Manager routes one next brief while `Product Goal Completion Status: Incomplete`, or records `Product Goal Completion Status: Complete`, non-empty `Product Goal Completion Evidence / Reference`, `Product Goal Stop Reason: Product Goal Complete`, `Product Iteration Loop Status: Stopped`, and `Next Iteration Status: Product Goal Complete` when the requirement is complete; routine human verification is not part of this loop.
