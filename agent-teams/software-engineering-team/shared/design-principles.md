...
- The existence of a method, state field, generic capability, fallback branch, synthetic reproduction, test-only caller, defensive mechanism, or ability to mutate internal files is not by itself evidence that a scenario occurs in the relevant production journey. An internal file is not a user-operated surface unless the product explicitly exposes it as one.
- Reachability requires a complete witness: a supported trigger or governing contract, the actual production caller/event path, the claimed state at the relevant lifecycle point, and the material consequence. Mechanical possibility at any one link is insufficient.
...
- Without a supported or observed journey or governing contract that completes the trigger-to-consequence witness, the premise cannot drive a finding or new machinery.
