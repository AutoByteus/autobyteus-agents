...
This is the shared practical design reference for the software engineering team.
...
- Write the primary execution/data-flow spine(s) as short arrow chains, but stretch each primary spine far enough to expose the real business path rather than only the local edited segment.
...
- In that inventory, `Primary End-to-End` means the primary top-level spine for one use case or major business path. There can be multiple primary rows.
- Add `Return-Event` rows when a callback, emitted event, or outward/upward propagation path matters to the behavior.
- Add `Bounded Local` rows when a loop, state machine, dispatcher, or worker cycle materially shapes one owner's internal behavior.
- Name the main-line nodes with natural domain language.
- Define what each node owns before splitting off-spine concerns.
- Apply the `Authoritative Boundary Rule`: if one boundary is the intended entrypoint for a domain subject, callers above it should use that boundary rather than also reaching into its internal manager, repository, helper, or lower-level concern dependencies.
...
- Apply the `Spine Span Sufficiency Rule`: the primary spine should usually show the initiating surface, the key boundary crossing/orchestration point, the authoritative owner, and the meaningful downstream effect. Practical default is `4-5` meaningful nodes unless the full path is genuinely smaller.
...
- Bad shape:
  ...
  - `Outer Boundary -> Internal Owned Mechanism`
- Rule:
  - `Authoritative Boundary Rule` (mandatory): when an outer boundary is the intended authority for a domain subject, callers above it should use that boundary rather than depend on both it and one of its internal owned mechanisms at the same time.
...
- This is the `no boundary bypass / no mixed-level dependency` rule. It is a general rule, not a naming rule. The same problem can appear as `service -> manager`, `facade -> runtime`, `controller -> orchestrator`, `use-case -> repository`, or many other label combinations.
...
## Spine Span Sufficiency Pattern
...
- Rule:
  - `Spine Span Sufficiency Rule` (mandatory): if the shorter local path hides who initiated the behavior, which boundary is authoritative, or what meaningful downstream consequence occurs, the primary spine is too short.
- Heuristic:
  - even when the code change touches only `1-3` nodes, the documented primary spine should usually still cover `4-5` meaningful nodes so the business path remains visible.
