...
5. If a concrete check produces a prospective finding or proposed or existing mechanism that depends on a material scenario outside the established behavior basis, first identify an independently supported initiating product path or governing contract, then complete the report's material-premise record using the shared product-reachability rule before accepting the conclusion. Reject a circular witness that uses a downstream technical mechanism or proposed design to establish its own initiating reachability. Do not search for hypothetical scenarios as a separate review stage.
