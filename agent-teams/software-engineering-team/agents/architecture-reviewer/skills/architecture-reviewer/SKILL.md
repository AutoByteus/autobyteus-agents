...
- When persisted data may be affected, verify that the design makes an evidence-backed transition decision rather than assuming migration from a schema change. Accept direct use or discard/rebuild when justified; when migration is required, verify its isolated ownership, ordering, validation, completion, interruption, and recovery behavior.
