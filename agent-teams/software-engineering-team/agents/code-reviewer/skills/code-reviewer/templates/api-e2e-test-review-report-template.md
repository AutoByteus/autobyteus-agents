...
A test may reproduce an established supported scenario, but a test caller,
fixture, endpoint, or synthetic setup cannot establish product-scenario validity
or a supported production path by itself. Do not create a finding from a
test-only or technically possible scenario without an independent product,
system, operational, or contract basis.
...
- `Requirement Gap`: intended behavior is missing or ambiguous; owned by `solution_designer` for requirements revision and any necessary renewed user approval
- `Unclear`: the issue cannot be classified from the available package; owned by `solution_designer` for routing
