...
- Do not treat Stage 9 as another design-review gate. The goal here is to synchronize long-lived docs to the final reviewed implementation state, not to reopen architecture review by default.
...
- If the final implementation state or intended behavior is still too unclear to document truthfully, block Stage 9 and route the issue explicitly instead of guessing in the docs.
