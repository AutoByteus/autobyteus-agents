...
- After rework, implementation-owned fixes return through source review and API/E2E; API/E2E-owned fixes return through execution and a proportional test-code review result (`Not Applicable` when no durable test changed).
...
- Each specialist owns one canonical revision record for its completed stage results: `solution-revision-record.md` (`SR-*`), `architecture-review-revision-record.md` (`ARCH-REV-*`), `implementation-revision-record.md` (`IR-*`), `code-review-revision-record.md` (`CRR-*`), `api-e2e-revision-record.md` (`API-REV-*`), and `delivery-revision-record.md` (`DR-*`). The owning specialist creates entry `001` for its first completed result and appends later entries to the same file.
- Canonical artifacts and code remain the current truth. Revision records are concise chronological indexes of the initial baseline, later delta, rationale, related findings, and routing; they do not duplicate or replace canonical content. A missing prior record or result is `N/A`, never an implied `Pass`.
- Each role creates and updates only its own artifacts. In particular, `solution_designer` does not create `implementation-handoff.md`; `implementation_engineer` creates it after implementation.
- After architecture review, the mandatory solution package and its still-relevant supplements together form the reviewed solution package. The design review report records the current gate decision, while the architecture review revision record preserves its round history.
- The package grows cumulatively in this order: requirements doc, investigation notes, design spec, relevant supplements, solution revision record, design review report, architecture review revision record, implementation handoff, implementation revision record, code review report, code review revision record, coverage investigation, execution coverage report, API/E2E revision record, API/E2E test review report, docs sync report, handoff summary, any release/deployment report, and delivery revision record. Each stage appends its output without dropping still-relevant upstream artifacts.
