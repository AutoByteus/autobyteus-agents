...
- Product Manager owns the Product Iteration Plan, ordered backlog/cursor, accepted history, rework/blocker history, next selected slice, and outer Product Iteration Loop Status. The plan is file-backed in the assigned workspace/artifact area when file access exists; otherwise the message body must carry the same state truthfully.
