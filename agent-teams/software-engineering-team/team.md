...
- After the initial handoffs, keep each role's latest canonical artifacts as the current truth. When a role revises completed work, maintain one role-owned revision record across later rounds: `solution-revision-record.md`, `implementation-revision-record.md`, `code-review-revision-record.md`, or `api-e2e-revision-record.md`. Each record explains the triggering report or finding, what changed, and why without duplicating the canonical artifacts.
- The package grows cumulatively in this order: requirements doc, investigation notes, design spec, relevant supplements, conditional solution revision record, implementation handoff, conditional implementation revision record, code review report, conditional code review revision record, coverage investigation, execution coverage report, conditional API/E2E revision record, API/E2E test review report, docs sync report, and delivery/release/deployment report. Each stage appends its output without dropping still-relevant upstream artifacts.
