...
- That file is the team's shared writing language for thesis quality, structure, evidence, style fidelity, bilingual fidelity, and platform fit.
...
- Default cumulative package:
  - `article_reviewer`: brief, source material or source article, style-profile choice, outline, current draft set
  - `article_writer` on revision return: brief, source material or source article, outline, current draft set, latest review report
...
2. `article_writer` records the requested platform, language, output mode, style profile, audience, one-sentence takeaway, depth target, and source-material basis in `brief.md`.
3. `article_writer` writes `outline.md` with title options, thesis, section purposes, and the evidence or examples each section will use.
...
- Review should prioritize thesis, structure, style fit, argument strength, fidelity, and platform fit before line-level polish.
