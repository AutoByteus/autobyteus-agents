...
## Example K2: Unsupported Contrast Anti-Pattern And Fix

### Bad: Invented Opposition

`Agent orchestration should start with teams, not control flow.`

### Why It Fails

- The source basis did not establish that anyone proposed `control flow` as the starting point.
- The negative side of the contrast is invented by the sentence.
- The title sounds sharp, but the sharpness comes from an unsupported opponent.
- This is an evidence failure, not only a tone problem.

### Better Ryan Builder-Direct

`In AutoByteus, we model multi-agent work as agent teams. A team defines its members, instructions, tools, skills, runtime configuration, and handoff rules. Member-to-member coordination happens through explicit messages and reference files.`

### Why The Fix Is Better

- Starts from the actual system instead of an invented debate.
- Preserves Ryan's directness and stable terminology without creating a false `not X` side.
- Explains the mechanism the source basis can support.
...
Use these only when the source basis supports the setup. They are sentence shapes, not permission to invent a visible problem, common belief, or directional conclusion.
