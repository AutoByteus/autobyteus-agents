...
- Ryan style can sharpen grounded claims, but it cannot invent the setup for those claims.
- Contrastive Ryan markers such as `not X, but Y`, `common explanation`, `should`, and `the deeper problem is` require an evidence anchor for the `X`, common belief, recommendation target, or visible problem.
- If no source establishes the contrast setup, use a direct builder or factual sentence instead.
...
3. evidence support for contrast, recommendation, and root-cause framing
4. ownership stance
5. logical flow
6. rhetorical pressure
7. sentence polish
...
Ryan's grounded `essay-thesis` writing often follows this sequence:

1. Open with a direct thesis or conclusion.
2. Name the common industry belief or common explanation when the source basis establishes one.
...
Do not use this as the default shape for factual technical, builder-direct, or paper-report drafts.
If no common belief or shallow explanation is grounded, skip the correction step and start from the supported system, observation, or mechanism.

The essay-thesis variant often feels like a controlled escalation:
...
- Use binary contrasts to sharpen meaning only when both sides are grounded: design vs validation, local cleanliness vs global structure, context-window limitation vs architectural weakness.
...
- Avoid any `not X, but Y` pivot when `X` is not established by the source basis.
...
19. No author-style marker has created an unsupported fact, opponent, common belief, recommendation, or root-cause claim.
