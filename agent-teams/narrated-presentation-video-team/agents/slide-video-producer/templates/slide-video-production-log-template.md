...
| Slide ID | Final Image Path | Final Image Tool (`generate_image`/`edit_image`) | Linked Segment ID(s) | Required Embedded Slide Content | Embedded Content Present And Correct? | Visual Quality / Engagement Check | Resource IDs Used | Readability Check | Source-Support Check | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SL-001 |  |  | SEG-001 |  |  |  |  |  |  |  |
...
| Slide ID | Accepted Image Attempt ID | Output Path | Visual Quality / Engagement Check | Mood / Style Check | Required Embedded Content Check | Inspection Result | `sleep 60` Completed Before Next Image Call? | Follow-Up |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SL-001 | IMG-SL001-A1 |  |  |  |  |  |  |  |

## Audio Production Log Summary

| Clip ID | Segment ID | Selected Narrator Identity | Audio Path | Prompt-Level Cues Used? | Voice Consistency Check | Inspection Result | `sleep 30` Completed Before Next Speech Call? | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| VO-001 | SEG-001 |  |  |  |  |  |  |  |
...
- Any accepted slide image low-quality, draft-like, generic, static, or visually dull: no
...
- Every `generate_speech` call was serial and followed by `sleep 30` after each result, failure, or timeout before the next speech call: yes
...
- `audio-generation-log.md` was updated immediately after every speech result, failure, timeout, rejection, or regeneration decision before cooldown and before the next speech call: yes
- Full image prompts/configs are recorded in `image-generation-log.md`: yes
- Full approved narration text, speech prompt text, prompt-level cues, selected narrator identity, voice-consistency checks, 30-second speech cooldown evidence, and configs are recorded in `audio-generation-log.md`: yes
...
- Every accepted image passes visual quality / engagement QA: yes
