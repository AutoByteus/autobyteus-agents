# Chapter Plan

## Purpose

- This file is a lightweight scoping artifact used before detailed boarding.
- It should define what the chapter is trying to do, but it should not try to replace `storyboard.md`.

## Scope Summary

- Production unit:
- Why this scope is the right first unit:
- Planned number of chapters in this pass:
- Storyboard status:

## Chapter Entry Template

### Chapter: [chapter-id]

- Working title:
- Narrative purpose:
- Opening state:
- Beat 1:
- Beat 2:
- Beat 3:
- Reveal or reversal:
- Ending beat or cliffhanger:
- Target page range:
- Target video-runtime range when voiced or video delivery is in scope:
- Must-render visual moments:
- Continuity-sensitive notes:
- Storyboard handoff note:
