...
- Present the brief to the user for approval before handing it to `architect`.
- Once the brief is approved, the only valid forward `send_message_to` recipient for that artifact is `architect`.
- When handing the approved brief to `architect`, include the artifact path, approval state, key scope summary, open risks, and the next expected decision.
